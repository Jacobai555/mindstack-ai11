...
let sessions = [];
const Route$2 = createFileRoute("/api/sessions")({
  server: {
    handlers: {
      GET: async () => {
...
const Route$1 = createFileRoute("/api/session-messages")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        const url = new URL(request.url);
        const sessionId = url.searchParams.get("sessionId");
        const { data } = await supabase.from("messages").select("*").eq("session_id", sessionId);
...
const LearnRoute = Route$7.update({
  id: "/learn",
  path: "/learn",
  getParentRoute: () => Route$9
});
const CyberLabRoute = Route$6.update({
  id: "/cyber-lab",
  path: "/cyber-lab",
  getParentRoute: () => Route$9
});
const ChatRoute = Route$5.update({
  id: "/chat",
  path: "/chat",
  getParentRoute: () => Route$9
});
...
const ApiSessionsRoute = Route$2.update({
  id: "/api/sessions",
  path: "/api/sessions",
  getParentRoute: () => Route$9
});
const ApiSessionMessagesRoute = Route$1.update({
