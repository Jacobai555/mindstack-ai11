...
import { Send, Square, Trash2, Brain, Plus, MessageSquare, Zap } from 'lucide-react'
...
function MessageBubble({ message }: { message: ChatMessages[number] }) {
  ...
  return (
    <div className={`flex gap-3 mb-4 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`max-w-[75%] px-4 py-3 text-sm ${isUser ? 'chat-bubble-user' : 'chat-bubble-ai'}`}
...
function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)]" style={{ color: 'var(--text-primary)' }}>
      {/* Sidebar */}
      <div className={`sidebar transition-all duration-300 flex-shrink-0 flex flex-col ${sidebarOpen ? 'w-64' : 'w-0 overflow-hidden'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="text-xs font-medium mb-2 px-2" style={{ color: 'var(--text-muted)' }}>Recent Chats</div>
          {messages.length > 0 ? (
            <div className="px-2 py-2 rounded-lg text-sm cursor-pointer"
              style={{ background: 'rgba(0,245,255,0.04)', border: '1px solid rgba(0,245,255,0.1)', color: 'var(--text-primary)' }}>
              <div className="flex items-center gap-2">
                <MessageSquare size={12} style={{ color: 'var(--neon-cyan)' }} />
                <span className="truncate text-xs">
                  {messages[0]?.parts[0]?.type === 'text' ? messages[0].parts[0].content?.slice(0, 30) + '…' : 'Chat session'}
                </span>
              </div>
            </div>
          ) : (
            <p className="text-xs px-2" style={{ color: 'var(--text-muted)' }}>No chats yet</p>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center px-4 text-center">
              <div className="ai-orb w-16 h-16 mb-6 flex items-center justify-center"
